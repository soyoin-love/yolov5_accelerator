...
module cacc_requant #(
    ...
    parameter ACC_W        = 25, 
    parameter BANK_NUM     = 16  // 对应 OBUF 16个 Bank
)(
    input  wire                               clk,
    input  wire                               rst_n,
    input  wire                               start,
    ...
    output reg  [BANK_NUM-1:0]                obuf_wr_mask,     // 16 bit 掩码，分别控制两组 FIFO
    output reg  [BANK_NUM*64-1:0]             obuf_wr_dat   
);
    ...
    reg [15:0] p1_mac_co_grp; // 将组号打拍对齐
    ...
    always @(posedge clk) begin
        if (requant_ready && quant_vld) begin
            ...
            p1_mac_co_grp <= mac_co_grp; // 随流水线寄存组号
            
            for (p = 0; p < PIXEL_NUM; p = p + 1) begin
                for (ch = 0; ch < TOTAL_OUT_CH; ch = ch + 1) begin
                    p1_mult[p*TOTAL_OUT_CH+ch] <= 
                        ( $signed(quant_data_in[(p*TOTAL_OUT_CH+ch)*ACC_W +: ACC_W]) * 
                        $signed(requant_params_1024b[ch*64 + 38 +: 18]) ) + 
    ...
    generate
        for (gp = 0; gp < PIXEL_NUM; gp = gp + 1) begin : P2_PIXEL
            for (gch = 0; gch < TOTAL_OUT_CH; gch = gch + 1) begin : P2_CH
                wire [7:0] shift_val = requant_params_1024b[gch*64 + 56 +: 8];
                wire signed [47:0] raw = p1_mult[gp*TOTAL_OUT_CH+gch] >>> shift_val;
                wire signed [15:0] min = cfg_relu_en ? 16'd0 : -16'd128;
                wire signed [15:0] c1  = (raw > 127) ? 127 : raw[15:0];
                wire signed [15:0] c2  = (c1 < min)  ? min : c1;
